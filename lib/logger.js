...
const catche = {};
function getLogger(categoryName, postfix) {
	if (catche[categoryName]) {
		return catche[categoryName];
	}
	...
	['log', 'mark', 'debug', 'info', 'warn', 'error', 'trace', 'fatal'].forEach(function (item) {
		pLogger[item] = function (...args) {
			const stacks = ErrorStackParser.parse(new Error());
			args.push(`${stacks[1].fileName.replace(process.cwd, '')}:${stacks[1].functionName}:${stacks[1].lineNumber}`);
			logger[item].call(logger, ...args);
		}
	});
	
	catche[categoryName] = pLogger;
	return pLogger;
};
 ...
 * Configure file just like log4js.json. And support ${scope: arg - name
